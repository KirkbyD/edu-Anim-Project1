...
cPhysicsManager::cPhysicsManager()
	: pMediator(nullptr), PhysicsFactory(0), hModule(0), PhysicsDLLIndex(0), PhysicsWorld(0), next_UniqueComponentID(0)
...
void cPhysicsManager::SwapPhysics() {
	...
	for (size_t i = 0; i < HotswapStructures.size(); ++i) {
		...
		if (PhysicsComponent != nullptr) {
			...
			WorldAddRigidBody(PhysicsComponent);
		}
	}
...
void cPhysicsManager::WorldUpdate(float dt) {
	...
	//Move animated entity for physics objects
	//Get all relevant objects per parent.
	std::map<cComplexObject*, std::vector<nPhysics::iPhysicsComponent*>> parentChildMap;
	for (auto component : this->PhysicsComponents) {
		//check for bone name
		std::string bone = component->GetBoneName();
		if (bone != "") {
			//has bone
			cComplexObject* pParent = component->GetParentPointer();
			parentChildMap[pParent].push_back(component);
		}
	}
	//resolve motion
	for (auto mapIt = parentChildMap.begin(); mapIt != parentChildMap.end(); mapIt++) {
		glm::vec3 average = glm::vec3(0.f);
		for (size_t i = 0; i < mapIt->second.size(); i++) {
			//shitty collision check, see if x or y are 0
			glm::vec3 compVel;
			mapIt->second[i]->GetVelocity(compVel);
			//collisions are currently perfetly inelastic
			if (compVel.x == 0 || compVel.z == 0) {
				average.x = 0.f;
				average.y = 0.f;
				break;
			}
			//if (compVel.y < 1.f) compVel.y = 0.f;
			average += compVel;
		}
		average /= mapIt->second.size();
		mapIt->first->MutatePosition(average);
		glm::vec3 newPos = mapIt->first->getPosition();
		if (newPos.y < 1.5f) {
			newPos.y = 1.5f;
			mapIt->first->SetPosition(newPos);
		}
		mapIt->second.clear();
	}
	parentChildMap.clear();

	//ensure everyone is still around...
	//for (auto component : this->PhysicsComponents) {
	//	cComplexObject* pParent = component->GetParentPointer();
	//	if (glm::length(pParent->getPosition()) > 500.f
	//		|| glm::length(pParent->getPosition()) == NAN) {
	//		//further than 500 total from origin, teleport back
	//		pParent->SetPosition(glm::vec3(0.f, 10.f, 0.f));
	//		component->SetPosition(glm::vec3(0.f, 10.f, 0.f));
	//	}
	//}
	return;
}
